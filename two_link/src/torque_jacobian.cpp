      ...
      priv_node_handle_("~")
{
  ...
  ************************************************************/
  ...
  initPoseFlag = true;   //초기값 맞으면 true로 바뀜
  checkFirstPoseFlag = true; // 한번훑고 false로 바뀜
  initPose.resize(6);
  PoseDelta.resize(6);
  firstPose.resize(6);
  X_cmd.resize(6);
  initPose << 0, 0.17, 0.35, 0, M_PI/2, 0;
}
...
void TorqJ::PublishCmdNMeasured()
{
  ...
  joint_measured.velocity.push_back(angle_ref[2]); 
  joint_measured.velocity.push_back(angle_ref[3]); 
  joint_measured.velocity.push_back(angle_ref[4]); 
  joint_measured.velocity.push_back(angle_ref[5]); 


  joint_measured.effort.push_back(FK_EE_pos[0]); 
  joint_measured.effort.push_back(FK_EE_pos[1]); 
  joint_measured.effort.push_back(FK_EE_pos[2]); 
  joint_measured.effort.push_back(FK_EE_ori[0]); 
  joint_measured.effort.push_back(FK_EE_ori[1]); 
  joint_measured.effort.push_back(FK_EE_ori[2]); 
...
void TorqJ::setInitpose()
{
  ...
  //init pose에 도달했나 확인
  if(abs(initPose[0] - FK_EE_pos[0]) <0.01 && abs(initPose[1] - FK_EE_pos[1]) <0.01 &&
     abs(initPose[2] - FK_EE_pos[2]) <0.01 && abs(initPose[3] - FK_EE_ori[0]) <0.01 && 
     abs(initPose[4] - FK_EE_ori[1]) <0.01 && abs(initPose[5] - FK_EE_ori[2]) <0.01)
  {
    initPoseCnt ++;
    if (initPoseCnt > 200) initPoseFlag = false;
